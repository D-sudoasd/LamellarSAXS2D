...
9. **导出图包或进入批处理**：保存各帧的 PONI、mask、q 范围、权重和配置。条件改变时一并解释其影响；几何已被观测支持时，可用上一帧作为下一帧初值，再对当前图像重新拟合。

固定 q 环路径的原始角向强度定义为有效像素的 `sum / count`，并同时保存 count 和 coverage；本测量层不追加固角、偏振或探测器效率校正。跨环算法从通过局部强度与 coverage 检测的实测峰中联合选择连续路径，以支持环数、相对 prominence 和角度平滑度排序；竞争峰仍可检查。少于连续 3 环或存在近似等支持路径时，轨迹标为低可信，但观测点仍保留。缺环会切断轨迹，不补造数据；`q_annulus` 是采样坐标，不是 `q*`，不能直接换算 `2π/q`。

annular 路径不把 q 窗口外边界当成长轴真实尖端，保留用户明确给出的参数上下界。长轴外推或短弧支持不足时，有限的 `a` 与派生周期以候选值显示，附带具体限制；它们与独立测得的环周期分开。完整判据与 CLI 选项见[固定 q 环花瓣轨迹](annular_trajectories_zh.md)。
