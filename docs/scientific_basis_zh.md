...
本页把当前 WingSAXS 源码实际计算的量，与两篇 Grubb 论文中的结构符号分开。论文给出科学背景和模型动机；软件输出以源码、公开 API 和结果 flags 为准。椭圆拟合或 `full2d` 成功，不等于已经完成唯一的三维结构反演。
...
自 0.4.3 起，已有实测点支持的有限拟合值会以估计值或候选值保留，并附带可信度和具体限制。未执行重采样、局部缺瓣或短弧支持不足不会自动清空这些值；真正缺失的观测、失败的求解和不可换算的单位仍单独标记。候选椭圆派生的 Ln/Lz 与独立径向峰给出的环周期分别保存，不互相替代。
