...
每个 q 环的角向 profile 保留全部通过强度与覆盖检测的候选峰；跨环选择只使用这些实测峰，不降低局部检测阈值。算法在相邻 q 环的候选图上联合选择路径，优先保留观测环数较多的连续支持，再按环内相对 prominence 与角度变化平滑度排序。每个参考象限输出一条选择轨迹，其余峰仍留在候选诊断中。

路径支持不足 3 个 q 环、存在得分接近的竞争路径，或相对环内最强峰的 prominence 较弱时，会将 `trajectory_confidence` 标为 `low`，并记录具体原因。短路径的观测点仍保留，但不会仅因达到该长度而自动视作充分的蝴蝶弧。缺少候选的 q 环或相邻环之间没有可行角度连接时会切开轨迹；算法不会跨缺环连接，也不会补造点。
